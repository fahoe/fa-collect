...
// UNIX Error reporting (cp from fa_samples.cpp)
...
static void *archthread (void *arg) {
   ...
   int debug_dev=(f->debug_switch)-1; 
   pabuf=&abuf[0][0];
   ...
   // printf("pabuf %x:%x:%x",abuf,&abuf,pabuf);
	...
	if(f->debug_switch && f->dlevel&0x10){
        ...
        for(i=0;i<MAX_LIB_THREADS;i++){
            //pn[i] = read (f->fd[i][0], &buf, sizeof(buf));
       	    //pn[i] = read (f->fd[i][0], (pbuf+i*BUF_SIZE), sizeof(buf));
...
/*   chid chan;
   //dbr_string_t value;
   dbr_long_t value;
   dbr_long_t *pval=&tmp_val;
   capri priority=0;

   SEVCHK(ca_context_create(ca_disable_preemptive_callback),"ca_context_create");
   printf("create channel %s for sumthread %d \n",f->sum, f->nb);
   SEVCHK(ca_create_channel(f->sum, NULL, NULL, priority, &chan),"Create channel failed");
   SEVCHK(ca_pend_io(5.0), "Search failed");
   /*-----------------------------------------*/
/*   snprintf (f->msg, BUF, "I'm SumThread Nr. %d: %ld", f->nb,pthread_self());
   printf(" Initialize SumThread: %s\n",f->msg);
   /*----------------------*/
...
/*    while(*(f->ex_flag)==0){
        ...
        psumbuf=&sumbuf[0];
       	...
       	pn = read (f->fdsum[0], sumbuf, 4); // 
        if (pn<0) {
           sprintf(err_string,"read sumpipe%d \t %d data read failed %d", 0,pn,errno);
           error(err_string, 0);
           close(f->fdsum[0]); /*close the read site of the pipe */
 /*          pthread_exit(f) ;
	} 
        //printf("sum dev %d received %d Bytes\n",f->nb,pn); 
        //printf("%d::%hhx\t",*psumbuf);
        //printf("%08x\t",*psumbuf);
	
        
        tmp_val = 0 ;
        for (j = 0 ; j < 4 ; j++){ 
            tmp_val |= ((unsigned char)sumbuf[j]) << (j*8) ;  
            //printf("%d:%d rcv temp_val\t %2x \t %016x\n",f->nb,j,*(psumbuf+j),tmp_val);

        }
        //printf("rcv temp_val \t %d\t %08x\t %20d\n",f->nb,tmp_val,tmp_val);
	SEVCHK(ca_put(DBR_LONG, chan, pval), "Put failed");
	ret=ca_pend_event ( 1.0 );
	ret=ca_poll ();
	sleep(1.0); 
    }
    printf("Exit SumThread Nr. %ld\n", pthread_self() );
    pthread_exit(f);
}
...
static void *libthread (void *arg) {
   ...
   char *prbuf;
   char *pwbuf;
   //made by main struct data int received = 0;
   //int samples=SAMPLES;
   int tmp_val;
   ...
   //
   // fahoe 20230118 - int setrcv_buf_size = 4096000;
   ...
   if ( getsockopt(server_sd, SOL_SOCKET, SO_RCVBUF, (char *)&rcv_buf_size,
    ...
    if (hostname) {
            ...
            sprintf(err_string,"Server: using  !: %s", hostname);
            error(err_string, 0);
    }
    else { // Use the host's own IP for server
            server_addr.sin_addr.s_addr = htonl(INADDR_ANY);
            sprintf(err_string,"Server: INADDR_ANY !: %s", hostname);
            error(err_string, 0);
    }   
    ...
    /*pn = fcntl( f->fdsum[1], F_SETFL, fcntl(f->fdsum[1], F_GETFL) | O_NONBLOCK);
    if (pn<0) {
      	sprintf(err_string,"write pipe of sumthread %d NONBLOCK failed %d\t nb:%d ", f->nb,errno,pn);
        error(err_string, 0);
    }*/
        ...
        if(f->debug_switch)
	{
        ...
        /*-------------- fahoe 20230116 ---------------*/
     /*
	  -we dont need photons va,vb,vc,d for the archiver, so scrab  the first 16 bytes 
	  - 16 ... 19 sum
            ...
            if(f->dlevel&0x04){
                ...
                for (i = 0 ; i < (BUF_SIZE/4) ; i++) {
              		...
              		for (j = 0 ; j < 4 ; j++) {
                        ...
                        printf("lib thread %d received data:%d\t %20d:%#08x\n",f->nb,i,tmp_val,tmp_val);            	
                }//rof BUF_SIZE
	...
	if (pn<0) {
        ...
        //printf("nb:%d\t%x\t%x\t%x\t%x\n",f->nb,*(wbuf+0),*(wbuf+1),*(wbuf+2),*(wbuf+3));

	//printf("Test5\n");
	//pn=write (f->fdsum[1], wbuf, 4);
	//printf("Test6\n");
        /* we use a nonblock pipe, so we dont check errors
	if (pn<0) {
        	sprintf(err_string,"write pipe of sumthread %d failed %d\t wrote data nb:%d ", f->nb,errno,pn);
                error(err_string, 0);
	}
        */
...
int main(void)
    ...
    pthread_t exth;
    pthread_t archth;
    //pthread_t sumth[MAX_LIB_THREADS];
    int i,j,p;
    int exit_flag=0;
    ...
    for (i = 0; i < MAX_LIB_THREADS; i++){
      ...
      if(ret[i] == NULL) {
        ...
        /*psum=pipe(ret[i]->fdsum);
        if(psum<0){
            printf("libera thread sum pipe allocation failed ...!:%d\n",i);
                exit(EXIT_FAILURE);
        }*/

        //strcpy(ret[i]->sum,sum[i]);
    ...
    /*for (i = 0; i < MAX_LIB_THREADS; i++) {
      if(pthread_create(&sumth[i],NULL,&sumthread,ret[i]) !=0) {
         fprintf (stderr, "sumthread creation failed\n");
         exit (EXIT_FAILURE);
      }
    ...
    /* watching for sumthreads */
    /*for( i=0; i < MAX_LIB_THREADS; i++)
      pthread_join(sumth[i], (void **)&ret[i]);
    
    printf("\n-> Main- all sumth stopped\n");*/
